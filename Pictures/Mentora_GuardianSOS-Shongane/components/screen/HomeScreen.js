...
import { useGroups } from '../../groupContext/groupContext'; // Add this import
...
export default function HomeScreen({ navigation }) {
  ...
  const { groups, fetchGroups, loading: groupsLoading } = useGroups(); // Add groups context
  ...
  // Generate community alerts from groups data
  const generateCommunityAlerts = (groupsData) => {
    if (!groupsData || groupsData.length === 0) return [];
    
    const alerts = [];
    
    groupsData.forEach(group => {
      // Alert for new groups you've joined
      if (group.isMember) {
        alerts.push({
          ...
          timestamp: group.joinedAt || new Date().toISOString(),
          icon: 'users'
        });
      }
      
      // Alert for active groups in your area
      if (group.memberCount > 0) {
        alerts.push({
          id: `group-active-${group._id}`,
          type: 'group_active',
          title: 'Active Community',
          message: `${group.name} has ${group.memberCount} members nearby`,
          groupName: group.name,
          memberCount: group.memberCount,
          timestamp: group.updatedAt || new Date(Date.now() - (Math.random() * 86400000)).toISOString(),
          icon: 'user-friends'
        });
      }
      
      // Alert for recent group messages
      if (group.messages && group.messages.length > 0) {
        const latestMessage = group.messages[group.messages.length - 1];
        alerts.push({
          id: `group-message-${group._id}-${latestMessage._id}`,
          type: 'group_message',
          title: 'Community Update',
          message: `New message in ${group.name}: ${latestMessage.content.substring(0, 50)}...`,
          groupName: group.name,
    ...
    // Sort by timestamp (newest first) and limit to 3 alerts
    return alerts
      .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))
      .slice(0, 3);
  };

  // Shake Detection SOS Functions
  const setupShakeListener = async () => {
    try {
      console.log('Setting up shake listener...');
      ...
      const sub = Accelerometer.addListener(accelerometerData => {
        if (isShakeSOSEnabled && AppState.currentState === 'active') {
          handleShakeDetection(accelerometerData);
        }
      });
      ...
      console.error('Error setting up shake listener:', error);
      Alert.alert('Error', 'Failed to setup shake detection');
      return null;
    }
  };

  const handleShakeDetection = (data) => {
    const { x, y, z } = data;
  ...
  const triggerSOSFromShake = async () => {
    ...
    Alert.alert(
      ...
      'Shake gesture detected! Sending SOS alerts to your emergency contacts with your location.',
      [{ text: "OK" }]
    );
    ...
    await sendSMSAndWhatsAppToContacts(locToSend);
  };

  const requestShakeSOSPermission = () => {
    Alert.alert(
      'Shake Gesture SOS Access',
      'Enable emergency SOS using shake gesture? Shake your phone 3 times quickly to trigger emergency alerts.\n\n⚠️ This will send SMS and WhatsApp messages to your emergency contacts with your location.',
      [
        {
          text: 'Cancel',
          style: 'cancel',
        ...
        {
          text: 'Enable Shake SOS',
          style: 'destructive',
          onPress: async () => {
            const sub = await setupShakeListener();
            if (sub) {
              setIsShakeSOSEnabled(true);
              Alert.alert(
                'Shake SOS Activated ✅',
                'Shake gesture SOS is now active!\n\nShake your phone 3 times quickly to trigger emergency SOS.\n\nThis will send SMS and WhatsApp messages to your emergency contacts.',
                [{ text: 'OK' }]
              );
            } else {
              Alert.alert(
                'Error',
                'Failed to activate shake SOS. Please try again.',
                [{ text: 'OK' }]
              );
            }
          }
  ...
  const buildSOSMessage = (loc) => {
    ...
    if (nearestPoliceStation) {
      nearestStationInfo = ` | Nearest police: ${nearestPoliceStation.name} (${(nearestPoliceStation.distance * 1000).toFixed(0)}m)`;
    }
    
    return `🚨 EMERGENCY SOS 🚨

${name} may be in DANGER and needs URGENT help!

📍 Current location: ${locText}${accuracyText}${nearestStationInfo}

🕐 Time: ${new Date().toLocaleString()}

⚠️ This is an automated emergency alert from Guardian SOS app.

Please check on them immediately and contact authorities if needed.`;
  };

  // Function to send SMS and WhatsApp
  const sendSMSAndWhatsAppToContacts = async (loc) => {
    ...
    const contactsWithPhones = contacts
      .map((c) => {
        ...
        return { contact: c, phone, name: c.name || 'Contact' };
      })
      .filter((c) => !!c.phone);
    ...
    for (let i = 0; i < contactsWithPhones.length; i++) {
      const { contact, phone, name } = contactsWithPhones[i];
      ...
      try {
        ...
        if (canOpenSMS) {
          ...
          console.warn(`Cannot send SMS to ${phone}`);
        }
      } catch (err) {
        console.warn(`Error sending SMS to ${name}:`, err);
      }
    }
    ...
    // Show confirmation
    if (smsSent) {
      Alert.alert('SOS Sent', 'Emergency SMS and WhatsApp messages have been sent to your contacts!');
    }
  };

  // Regular SOS activation function
  ...
  const handleSOSPress = () => {
    Alert.alert(
      ...
      [
        ...
        { 
          text: 'Shake Gesture SOS', 
          onPress: () => {
            if (isShakeSOSEnabled) {
              Alert.alert(
                "Shake SOS Active",
                "Shake gesture SOS is already active. Shake your phone 3 times quickly to trigger emergency.",
                [{ text: "OK" }]
              );
            } else {
              requestShakeSOSPermission();
            }
          },
          style: 'default'
        },
    ...
    const initLocation = async () => {
      try {
        ...
        if (status === 'granted') {
          ...
          setHomeWatcher(locationSubscription);
        }
      } catch (err) {
  ...
  // Fetch contacts data with alert tracking
  useEffect(() => {
    const fetchContactsData = async () => {
      ...
      try {
        ...
        if (res.ok && Array.isArray(data.contacts)) {
          ...
          // ✅ Create alerts for existing contacts
          const contactAlerts = data.contacts.slice(0, 2).map(contact => ({
            id: generateId(),
            type: 'contact_loaded',
            title: 'Emergency Contact',
            message: `${contact.name} is your emergency contact`,
            contactName: contact.name,
            timestamp: new Date().toISOString(),
            icon: 'user'
          }));
          
          setContactUpdates(contactAlerts);
        } else {
  ...
  // Load existing contacts as alerts when component mounts
  useEffect(() => {
    const loadExistingContactsAsAlerts = async () => {
      if (userContacts.length > 0) {
        // Create alerts for existing contacts (with a recent timestamp)
        const existingContactAlerts = userContacts.slice(0, 2).map(contact => ({
          id: generateId(),
          type: 'contact_added',
          title: 'Emergency Contact',
          message: `${contact.name} is your emergency contact`,
          contactName: contact.name,
          timestamp: new Date(Date.now() - (Math.random() * 86400000)).toISOString(), // Random recent time
          icon: 'user'
        }));
        
        setContactUpdates(existingContactAlerts);
      }
    };

    loadExistingContactsAsAlerts();
  }, [userContacts]);

  // Fetch groups and generate community alerts
  useEffect(() => {
    const loadGroups = async () => {
      try {
        await fetchGroups();
      } catch (error) {
        console.log('Could not load groups:', error.message);
      }
    };
  ...
  // Generate community alerts when groups data changes
  useEffect(() => {
    if (groups.length > 0) {
      const alerts = generateCommunityAlerts(groups);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.userWelcome}>
          ...
          {/* Shake SOS Status */}
          <View style={[
            styles.shakeSOSIndicator,
            isShakeSOSEnabled ? styles.shakeSOSEnabled : styles.shakeSOSDisabled
          ]}>
            <Icon 
              name={isShakeSOSEnabled ? "mobile" : "mobile-alt"} 
              size={14} 
              color={isShakeSOSEnabled ? "#4CAF50" : "#FF6B6B"} 
            />
            <Text style={styles.shakeSOSIndicatorText}>
              {isShakeSOSEnabled ? "Shake SOS: Active" : "Shake SOS: Inactive"}
            </Text>
            <TouchableOpacity 
              onPress={isShakeSOSEnabled ? null : requestShakeSOSPermission}
              style={styles.shakeSOSButton}
            >
              <Text style={styles.shakeSOSButtonText}>
        ...
        <View style={styles.statsGrid}>
          ...
          {/* Communities Card */}
          <TouchableOpacity 
            style={[styles.statCard, groups.length === 0 && styles.emptyStatCard]}
            activeOpacity={0.7}
          >
            <Text style={styles.statValue}>{groups.length}</Text>
            <Text style={styles.statLabel}>Communities</Text>
            {groups.length === 0 && (
              <Text style={styles.addHint}>Tap to join</Text>
            )}
            {groups.length > 0 && (
              <Text style={styles.viewHint}>Tap to view</Text>
            )}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.alertList}>
            {/* Safety Zone Update - Keep this one as it's based on real safety data */}
            ...
            {/* Community Alerts - Dynamic from your groups */}
            {communityAlerts.length > 0 ? (
              ...
              /* Fallback when no community alerts */
              <TouchableOpacity 
                style={styles.alertItem}
              >
                <View style={styles.alertIcon}>
                ...
                <View style={styles.alertContent}>
                  ...
                  <Text style={styles.alertText}>
                    Join communities to receive neighborhood safety updates and alerts
                  </Text>
                  <Text style={styles.alertTime}>Tap to explore</Text>
                </View>
              </TouchableOpacity>
            )}

            {/* Emergency Contact Updates - Dynamic from your contacts */}
            {contactUpdates.length > 0 ? (
              contactUpdates.slice(0, 1).map((alert) => (
                <View key={alert.id} style={styles.alertItem}>
                  <View style={styles.alertIcon}>
                    <Icon 
                      name={alert.icon === 'user-plus' ? 'user-plus' : 'user'} 
                      size={16} 
                      color="#2A5B8C" 
                    />
                  </View>
              ...
              /* Fallback when no contact updates yet */
              <View style={styles.alertItem}>
                <View style={styles.alertIcon}>
                ...
                <View style={styles.alertContent}>
                  ...
                  <Text style={styles.alertText}>
                    You have {contactsCount} emergency contact{contactsCount !== 1 ? 's' : ''} set up
                  </Text>
                  <Text style={styles.alertTime}>
                    {contactsCount === 0 ? 'Add contacts for safety' : 'All set'}
                  </Text>
                </View>
              </View>
            )}
      ...
      <TouchableOpacity style={styles.sosButton} onPress={handleSOSPress}>
        <View style={styles.sosButtonInner}>
          ...
          <View style={[styles.sosPermissionIndicator, 
            { backgroundColor: isShakeSOSEnabled ? '#4CAF50' : '#FF6B6B' }]} />
        </View>
      </TouchableOpacity>
    </SafeAreaView>
  );
