...
const NeighborhoodWatchScreen = () => {
  ...
  // This is used again to test the api Test API connection-It can be removed once confirmed working
  ...
  const testAPIConnection = async () => {
    setTestingAPI(true);
    try {
      console.log('🧪 Testing API connection to: http://192.168.137.1:3000');
      const response = await fetch('http://192.168.137.1:3000');
      console.log('✅ API Test Response:', response.status, response.statusText);
      Alert.alert('API Test Success', `Status: ${response.status}\nServer is running!`);
    } catch (error) {
      console.error('❌ API Test Failed:', error);
      Alert.alert('API Test Failed', 
        `Cannot connect to backend server.\n\nMake sure:\n• Backend is running on 192.168.137.1:3000\n• Your phone and computer are on same WiFi\n• No firewall blocking the connection\n\nError: ${error.message}`
      );
    } finally {
      setTestingAPI(false);
    }
  ...
  const handleJoinGroup = async (group) => {
    const updatedData = {
      ...
      membersOnline: (group.membersOnline || 0) + 1,
    };
        ...
        { 
          ...
          onPress: async () => {
            const updatedData = {
              ...
              membersOnline: Math.max(0, (group.membersOnline || 0) - 1),
            };
            await updateGroup(group._id, updatedData);
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        ...
        {/* This button is used to test if the APIs are working to fetch the group data - TEMPORARY */}
        <TouchableOpacity 
          style={[styles.createGroupButton, { backgroundColor: '#8B5CF6', marginTop: 10 }]}
        ...
        >
          {testingAPI ? (
            ...
            <FontAwesome name="wifi" size={18} color="white" />
          )}
      ...
      <View style={{ marginBottom: 20 }}>
        ...
        {loading ? (
          ...
          <View style={styles.errorContainer}>
            ...
            <Text style={styles.errorSubtext}>Cannot connect to server. Check your connection.</Text>
